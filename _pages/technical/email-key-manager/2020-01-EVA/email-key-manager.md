...
| Command | Description |
|:---------|:---------|
| `--test-store` | Test connection to HSM or KMS, then quit |
| `--export=<file>` | **NOT IMPLEMENTED** Export store contents to a json file, then exit. The exported data file is NOT encrypted, and therefore very sensitive. |
| `--import=<file>` | **NOT IMPLEMENTED** Import JSON data file into the store, then exit |
...
To test your overall store settings, run `java -jar flowcrypt-email-key-manager.jar --test-store`
...
You can debug more complex issues by running with a `--log-level=FINE` sysarg. For help or to report issues, please email `human@flowcrypt.com`
